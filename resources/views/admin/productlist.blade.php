    ...
    <table id="productTable" class="display table table-bordered text-center">
        ...
        @foreach($products as $p)
            <tr>
                        ...
                        @foreach($p->colors as $color)
                            @foreach($color->images ?? [] as $img)
                                <img src="{{ asset('storage/colors/' . $img->img_path) }}"
                                     alt="{{ $img->img_alt_text }}" width="40px" class="rounded border preview-img">
                            @endforeach
                        @endforeach
                    </div>
                </td>
                ...
                <!-- Actions -->
                <td>
                    <a href="{{ route('product.view', $p->p_id) }}" class="btn btn-sm btn-orange mb-1">View</a>
                    <a href="{{ route('product.edit', $p->p_id) }}" class="btn btn-sm btn-orange mb-1">Edit</a>
